...
export default class MultiSelectCombobox extends LightningElement {
  ...
  handleChange(event) {
    ...
    if (selectedValue === 'All') {
      ...
      this.currentOptions.find((item) => item.value === 'All').selected = true;
    } else {
      // Handle other options
      const allOption = this.currentOptions.find((item) => item.value === 'All');
      if (allOption.selected) {
        // Deselect "All" when another option is selected
    ...
    if (selectedValue === 'None') {
      // Handle "All" option selection
      this.currentOptions.forEach((item) => (item.selected = false));
      this.currentOptions.find((item) => item.value === 'None').selected = true;
    } else {
      // Handle other options
      const allOption = this.currentOptions.find((item) => item.value === 'None');
      if (allOption.selected) {
        // Deselect "All" when another option is selected
        allOption.selected = false;
      }
